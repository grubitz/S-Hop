...
class Category
{
    ...
    {
        $db = new Database();
        $query = "SELECT c.*, COUNT(p.id) AS product_count FROM products p RIGHT JOIN categories c ON p.category_id = c.id
                  GROUP BY c.id ORDER BY c.parent_category_id, c.name";
